...
class AdvanceExpenseLine(models.Model):
    # ORM ------------------------------------------------------------------------------------ ORM #
    _translate = True
    _inherit = ['advance.expense.line']                 # ['mail.thread','other.model'] for oppenchatter
    ...
    def get_header_adv1(self, head_id, detail_id, type, detail_id2=False):
        ...
        for row in data:
            total += row.total_amount
            total_real += row.realisasi_amount
        ...
        if detail_id2:
            ...
            for row in data2:
                ...
                total2_real += row.realisasi_amount

        if type == 'header1':
            result = total
        if type == 'header2':
            result = total2
        if type == 'header1_real':
            result = total_real
        if type == 'header2_real':
            result = total2_real
        return result
    ...
    def get_header_adv2(self, head_id, detail_id, type, detail_id2=False):
        ...
        total2_real = 0
        if detail_id2:
            data2 = self.env['advance.expense.line.detail2'].search([('advance_line_id', '=', head_id), ('detail1', '=', detail_id), ('detail2', '=', detail_id2)])
            for row in data2:
                total2 += row.total_amount
                total2_real += row.realisasi_amount
        ...
        for kertas in self.advance_expense_line_ids:
            ...
            for rengar_output in kertas.rengar_output:
                if output_compare != rengar_output.name:
                    worksheet.write(row, 0, kertas.rengar_kegiatan.code + '.' + rengar_output.code, italic_border_style)
                ...
                if suboutput_compare != rengar_suboutput.name:
                    worksheet.write(row, 0, kertas.rengar_kegiatan.code + '.' + rengar_output.code + '.' + (rengar_suboutput.code or ''), italic_border_style)
            ...
            for paper_adv in paper.advance_expense_line_ids:
                ...
                for paper_adv_adv in paper_adv.advance_expense_line_ids:
                    if header_1 != paper_adv_adv.detail1.name:
                        ...
                        for x in range(3, 8):
                            worksheet.write(row, x, "", bold_border_style)
                        worksheet.write(row, 8, self.get_header_adv1(paper_adv.id, paper_adv_adv.detail1.id, 'header1'), bold_just_money_border_style)
                    ...
                    if header_2 != paper_adv_adv.detail2.name:
                        ...
                        for x in range(3, 8):
                            worksheet.write(row, x, '', bold_border_style)
                        worksheet.write(row, 8, self.get_header_adv1(paper_adv.id, paper_adv_adv.detail1.id, 'header2', paper_adv_adv.detail2.id), bold_just_money_border_style)
